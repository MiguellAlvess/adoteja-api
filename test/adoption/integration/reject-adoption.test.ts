...
import { RejectAdoption } from "../../../src/application/usecase/adoption/reject-adoption.js"
import type {
  PhotoInput,
  PhotoStorage,
...
describe("Reject Adoption", () => {
  ...
  let rejectAdoption: RejectAdoption
  ...
  beforeAll(async () => {
    ...
    rejectAdoption = new RejectAdoption(adoptionRepository)
  })
  ...
  test("should reject a PENDING adoption", async () => {
    const ownerOutput = await signup.execute({
      name: "Owner",
      email: `owner-rj-${Date.now()}@example.com`,
    ...
    const adopterOutput = await signup.execute({
      name: "Adopter",
      email: `adopter-rj-${Date.now()}@example.com`,
      password: "ValidPassword123",
      phone: "(83) 91111-0000",
      city: "Campina Grande",
      state: "PB",
    ...
    const rejectOutput = await rejectAdoption.execute(requestOutput.adoptionId)
    expect(rejectOutput.id).toBe(requestOutput.adoptionId)
    expect(rejectOutput.status).toBe("REJECTED")
    expect(rejectOutput.completedAt).toBeNull()
    ...
    expect(adoptionRow?.completedAt).toBeNull()
  })

  test("should throw AdoptionNotFoundError when id does not exist", async () => {
    await expect(
      rejectAdoption.execute("ac355f06-db61-4e5b-8fef-7e6abb6ce47c")
